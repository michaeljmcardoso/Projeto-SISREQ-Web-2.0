...
def pagina_editar():
    ...
    item_id = st.number_input("Digite o ID do Processo para Editar", min_value=1, step=1) # Entrada do ID para edição
    ...
    if registro:
        ...
        with col2:
            ...
            if new_data_certificacao.strip():  
                if re.match(r'^\d{2}-\d{2}-\d{4}$', new_data_certificacao.strip()): # Regex para verificar se a data está no formato DD-MM-YYYY
        ...
        with col4:
            ...
            if new_portaria_dou_input.strip():  
                if re.match(r'^\d{2}-\d{2}-\d{4}$', new_portaria_dou_input.strip()): # Regex para verificar se a data está no formato DD-MM-YYYY
            ...
            if new_decreto_dou_input.strip():  
                if re.match(r'^\d{2}-\d{2}-\d{4}$', new_decreto_dou_input.strip()): # Regex para verificar se a data está no formato DD-MM-YYYY
        ...
        if new_data_sentenca.strip():  
            if re.match(r'^\d{2}-\d{2}-\d{4}$', new_data_sentenca.strip()): # Regex para verificar se a data está no formato DD-MM-YYYY
